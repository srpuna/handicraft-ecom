...
@section('content')
    <div class="max-w-xl mx-auto bg-cream p-6 rounded-lg shadow">
        <form action="{{ route('admin.shipping.zones.update', $zone) }}" method="POST">
            ...
            <div class="mb-4">
                <label class="block text-sm font-medium text-truffle-extra-dark">Zone Name</label>
            ...
            <div class="mb-4">
                <label class="block text-sm font-medium text-truffle-extra-dark">Countries (Comma Separated)</label>
                <textarea name="countries" rows="3"
                    class="mt-1 block w-full rounded-md border-truffle-medium/30 shadow-sm focus:border-green-500 focus:ring-green-500 border p-2"
                    required>{{ implode(',', $zone->countries) }}</textarea>
                <p class="text-xs text-truffle-extra-dark mt-1">E.g. US, CA, UK</p>
            </div>
