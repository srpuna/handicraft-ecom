    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-bold flex items-center gap-2">
                {{ $client->name }} <span
                    class="text-sm font-mono text-gray-500 bg-gray-100 px-2 py-0.5 rounded">{{ $client->buyer_id }}</span>
            </h2>
        ...
        <div class="xl:col-span-1 space-y-6">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    ...
                    <span class="text-xs text-gray-400">Since {{ $client->created_at->format('M Y') }}</span>
                </div>
                ...
                <div class="p-6 space-y-4 text-sm text-gray-700">
                    ...
                    <div class="flex justify-between items-start">
                        ...
                        <span class="text-right"><a href="mailto:{{ $client->email }}"
                                class="text-green-600 hover:underline">{{ $client->email ?? '-' }}</a></span>
                    </div>
                    <div class="flex justify-between items-start">
                    ...
                    <div class="pt-4 border-t border-gray-100">
                        ...
                        <address
                            ...
                            @if(!$client->full_address) <span class="text-gray-400 italic">No address recorded</span> @endif
                        </address>
                    </div>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4">
                                        <a href="{{ route('admin.orders.show', $o) }}"
                                            class="font-mono font-semibold text-green-700 hover:underline">{{ $o->order_number }}</a>
                                        @if($o->is_paid) <span
                                    ...
                                    <td class="px-6 py-4 text-gray-600">{{ $o->created_at->format('d M Y') }}</td>
                                    <td class="px-6 py-4">
                                        @php
