...
<article class="container mx-auto px-4 sm:px-6 py-12">
    ...
    <nav class="mb-8 text-sm">
        <ol class="flex flex-wrap items-center gap-2 text-truffle-extra-dark">
            <li><a href="{{ route('home') }}" class="hover:text-green-premium">Home</a></li>
            <li><span>/</span></li>
            <li><a href="{{ route('blog.index') }}" class="hover:text-green-premium">Blog</a></li>
            <li><span>/</span></li>
            <li class="text-truffle-extra-dark truncate max-w-xs">{{ $post->title }}</li>
        </ol>
    </nav>

    <!-- Article Header -->
    <header class="max-w-4xl mx-auto mb-12">
        <h1 class="text-4xl md:text-5xl font-serif font-bold text-truffle-extra-dark mb-6 leading-tight">
            {{ $post->title }}
        </h1>

        <!-- Meta Info -->
        <div class="flex flex-wrap items-center gap-6 text-truffle-extra-dark mb-8">
            @if($post->author)
                <div class="flex items-center gap-2">
                    <div class="w-10 h-10 rounded-full bg-green-premium/20 flex items-center justify-center">
                        <span class="text-green-premium font-medium">{{ substr($post->author->name, 0, 1) }}</span>
                    </div>
        ...
        <!-- Featured Image -->
        @if($post->featured_image)
            <div class="aspect-[2/1] rounded-2xl overflow-hidden bg-[#F5F2EA]">
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <!-- Share Buttons -->
        <div class="mt-12 pt-8 border-t border-truffle-medium/30">
            <h3 class="text-lg font-semibold text-truffle-extra-dark mb-4">Share this article</h3>
            <div class="flex flex-wrap gap-3">
                ...
                <a href="https://wa.me/?text={{ urlencode($post->title . ' - ' . route('blog.show', $post->slug)) }}" 
                    target="_blank" rel="noopener noreferrer"
                    class="flex items-center gap-2 px-4 py-2 bg-green-premium text-white rounded-lg hover:bg-green-premium transition">
    ...
    @if($relatedPosts->count() > 0)
        <section class="mt-16 pt-16 border-t border-truffle-medium/30">
            <h2 class="text-2xl font-serif font-bold text-truffle-extra-dark mb-8 text-center">Related Articles</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @foreach($relatedPosts as $related)
                    <article class="bg-cream rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition border border-gray-100">
                        <a href="{{ route('blog.show', $related->slug) }}" class="block aspect-[16/10] overflow-hidden bg-[#F5F2EA]">
                        ...
                        <div class="p-5">
                            <p class="text-sm text-truffle-extra-dark mb-2">{{ $related->published_at->format('M d, Y') }}</p>
                            <h3 class="font-serif font-bold text-truffle-extra-dark line-clamp-2">
                                <a href="{{ route('blog.show', $related->slug) }}" class="hover:text-green-premium transition">
                                    {{ $related->title }}
                                </a>
                            </h3>
                        </div>
